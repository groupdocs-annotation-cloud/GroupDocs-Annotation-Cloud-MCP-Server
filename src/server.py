...
import groupdocs_annotation_cloud as sdk
...
mcp = FastMCP("mcp-groupdocs-annotation-cloud", host=_mcp_host, port=_mcp_port)
...
@mcp.tool()
def annotation_supported_formats() -> list[SupportedFileFormat]:
...
@mcp.tool()
def annotation_get_info(file_path: str, storage_name: str | None = None) -> dict:
    """Return basic document info (path, extension, size, etc.) for a file in cloud storage."""
    info_api = sdk.InfoApi.from_config(configuration)
    file_info = sdk.FileInfo(file_path=file_path, storage_name=storage_name)
    result = info_api.get_info(sdk.GetInfoRequest(file_info))
    return _to_dict(result)


@mcp.tool()
def annotation_extract(file_path: str, storage_name: str | None = None) -> list[dict]:
    """Extract annotations from a document in cloud storage. Returns a list (may be empty)."""
    annotate_api = sdk.AnnotateApi.from_config(configuration)
    file_info = sdk.FileInfo(file_path=file_path, storage_name=storage_name)
    result = annotate_api.extract(sdk.ExtractRequest(file_info))
    items = result or []
